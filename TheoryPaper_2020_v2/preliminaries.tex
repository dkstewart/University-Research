...
Our approach in computing minimal cut sets through the use of inductive validity cores is to supply activation literals constrained to be false to the algorithm. The resulting $\mathit{MCS}s$ consist of elements $\neg \mathit{af}_i$. The removal of this constraint from the constraint system results in true activation literals. We use the notation $\mathit{MCS}_\top$ to denote the set $\mathit{MCS}$ with all false constraints removed from its elements.

\begin{theorem}$\mathit{MCS}_\top = S$ for a minimal cut set $S$ of $\neg P$.
\begin{proof}
