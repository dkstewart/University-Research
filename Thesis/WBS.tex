...
Notice that the computation time includes the nominal model analysis through AGREE, the IVC computation time through JKind, and the minimal cut set generation time through the safety annex {\em for all top level properties} being verified in parallel. The time from cardinality $c = 4$ onward is noticeably increased as the number of cut sets increases exponentially -- likely due to the 8 wheel architecture. 
...
\danielle{Still need to re-run prob analysis and collect timing results. Cannot find my last run data. :( Will enter times and add a short discussion on these along with a comparison discussion with the max n cardinality timing results.}
