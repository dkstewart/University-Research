...
In a compositional proof, the analysis is performed top down. Given the assumptions, a property $P$ is proven (or disproven) based on the guarantees at the lower level: $gf_1, \dots, gf_n \vdash P$. The next level of analysis iterates through each guarantee and proves it by use of the guarantees $gf'$ at the next lower level: $gf'_1, \dots, gf'_k \vdash gf_1$, and $gf'_1, \dots, gf'_k \vdash gf_2$, and so on. This process continues until the leaf level guarantees been proven and there are no lower level guarantees. Similarly with MIVC generation, and by extension MCS generation, each guarantee at higher levels will have their own MCSs. Notationally, we refer to these as $MCS(gf_i)$. For a safety property $P$, the set of all MCSs are understood as $\lor^{n}_{i=1} MCS(P)$. For each element $gf_j \in MCS_i$, this is understood as $\land^{m}_{i=1} gf_i$. The composition of these results is performed top down and shown in Algorithm~\ref{alg:compose}. For each guarantee found in an MCS, a replacement must be made with its own MCSs. This is done recursively until all replacements have been made. At that time, the formula must be converted back into disjunctive normal form to finish the translation into minimal cut sets. The \texttt{init} method (line 1) is called once per property per layer of analysis after the conversion of MIVCs into MCSs for that property.
